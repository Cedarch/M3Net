        ...
        for key, val in data_dict.items():
            try:
                if key in ['voxels', 'voxel_num_points']:
                    ...
                    for bs_idx in range(len(data_dict['image_sequence'])):
                        # assert len(data_dict['image_sequence']) ==1
                        for i in range(len(data_dict['image_sequence'][bs_idx])):
                            for key, val in data_dict['image_sequence'][bs_idx][i].items():
                                if key in ['camera_imgs']:
                                    camera_imgs_list.append(torch.stack(val,dim=0))
                                elif key =='camera_intrinsics':
                      
                                    camera_intrinsics_list.append(np.stack([v.reshape(4,4) for v in val],axis=0))
  
                                elif key =='camera2lidar':
                                    try:
                                        camera2lidar_list.append(np.stack([v.reshape(4,4) for v in val],axis=0))
                                    except:
                                        import pdb;pdb.set_trace()
                                elif key =='img_aug_matrix':
                                    try:
                                        img_aug_matrix_list.append(np.stack([v.reshape(4,4) for v in val],axis=0))
                                    except:
                                        import pdb;pdb.set_trace()
        
                                elif key =='lidar2image':

                                    try:
                                        lidar2image_list.append(np.stack([v.reshape(4,4) for v in val],axis=0))
                                    except:
                                        import pdb;pdb.set_trace()

                    ret['camera_imgs'] = torch.stack(camera_imgs_list, axis=0)
                    ret['camera2lidar'] = np.stack(camera2lidar_list, axis=0)
                    ret['img_aug_matrix'] = np.stack(img_aug_matrix_list, axis=0)
                    ret['camera_intrinsics'] = np.stack(camera_intrinsics_list, axis=0)
                    ret['lidar2image'] = np.stack(lidar2image_list, axis=0)
                elif key in ['calib']:
                    ret[key] = val
