...
if True:
    ...
    class ChannelScan(torch.autograd.Function):
        @staticmethod
        def forward(ctx, x: torch.Tensor):
            B, C, H, W = x.shape
            ctx.shape = (B, C, H, W)
        ...
        def backward(ctx, ys: torch.Tensor):
            ...
            L = H * W
            # ys = ys[:, 0:2] + ys[:, 2:4].flip(dims=[-1]).view(B, 2, -1, L)
            y = ys[:, 0] + ys[:, 1].view(B, -1, W, H).flip(-1).contiguous().view(B, -1, L)
            return y.view(B, -1, H, W)
    ...
    class ChannelMerge(torch.autograd.Function):
        @staticmethod
        def forward(ctx, ys: torch.Tensor):
            B, K, D, H, W = ys.shape
            ctx.shape = (H, W)
            ys = ys.view(B, K, D, -1)
            # ys = ys[:, 0:2] + ys[:, 2:4].flip(dims=[-1]).view(B, 2, D, -1)
            y = ys[:, 0] + ys[:, 1].view(B, -1, W, H).flip(-1).contiguous().view(B, D, -1)
            return y
        ...
        def backward(ctx, x: torch.Tensor):
            ...
            xs[:, 0] = x
            xs[:, 1] = x.view(B, C, H, W).flip(-1).flatten(2, 3)
            # xs[:, 2:4] = torch.flip(xs[:, 0:2], dims=[-1])
...
class SS2DQuery(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # x proj; dt proj ============================
        self.K = 2 if not (self.forward_core == self.forward_corev1_share_ssm) else 1
        self.x_proj = [
            nn.Linear(self.d_inner, (self.dt_rank + self.d_state * 2), bias=False, **factory_kwargs)
            for _ in range(self.K)
        ]
...
class SSChannel(nn.Module):
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        if self.d_conv > 1:
            ...
            x = x.reshape(1,-1,x.shape[-1]).permute(1,0,2).unsqueeze(-1) # (b, 1, 256, 1)
            # x = x.permute(0, 3, 1, 2).contiguous()
            y = self.forward_core(x)
            # y = y * z
        # out = self.dropout(self.out_proj(y))
...
class VSSBlock(nn.Module):
    ...
    def _forward(self, input: torch.Tensor):
        ...
        if self.use_channel_mamba:
            x = x + self.drop_path(self.op_channel(self.norm2(x))).reshape(1,180,180,-1) # FFN
        elif self.mlp_branch:
...
class VSSM(nn.Module):
    def __init__(
        ...
        # weight_init.c2_xavier_fill(self.map_features)
        # weight_init.c2_xavier_fill(self.heat_features)

        self.heat_features = SElayer(self.dims[0])
        self.map_features = SElayer(self.dims[0])
        # map_grid_conf = {
        ...
        self.mapslicer = BevFeatureSlicer(grid_conf=grid_conf,map_grid_conf=motion_grid_conf)
    ...
    def forward(self, x: torch.Tensor):
        ...
        map_bev = self.map_features(out)
        map_features = self.mapslicer(map_bev).transpose(2,3)
        heat_features = self.heat_features(out)
        line_features = None
        mask_features = None
        mask_feat_bevsize, map_feat_bevsize,map_features_list, heat_features_list, map_feat_list = None,None,None,None,None
        transformer_encoder_features = [out]
        multi_scale_features = [out]
        return mask_features,line_features, map_features, transformer_encoder_features, multi_scale_features,mask_feat_bevsize, \
               map_feat_bevsize,heat_features, map_features_list, heat_features_list, map_feat_list
...
class VSSBlockENC(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.config = config
        self.norm = norm_layer(hidden_dim)
        self.norm_channel = norm_layer(hidden_dim)
        self.op = SS2D(
            d_model=hidden_dim, 
            dropout=attn_drop_rate, 
            d_state=d_state, 
        ...
        if self.config.get('MAMBA_WITH_CA',False):
            self.op3 = SS2DQuery(
                d_model=hidden_dim, 
                dropout=attn_drop_rate, 
                d_state=d_state, 
        ...
        self.drop_path = DropPath(drop_path)
        self.use_channel_gate = self.config.USE_TCS
        if self.use_channel_gate:
            self.gate = GateFFNLayer(hidden_dim,config)
        
        self.mlp_branch = mlp_ratio > 0
        mlp_hidden_dim = int(hidden_dim * mlp_ratio)
        # if self.mlp_branch:
        # self.norm2_det = norm_layer(hidden_dim)
        # self.norm2_seg = norm_layer(hidden_dim)
        # self.mlp_det = Mlp(in_features=hidden_dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop, channels_first=False)
        # self.mlp_seg = Mlp(in_features=hidden_dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop, channels_first=False)
        self.norm2 = norm_layer(hidden_dim)
        # self.mlp = Mlp(in_features=hidden_dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop, channels_first=False)

    def _forward(self, input: torch.Tensor, top_index, xy_index, query,layer_index):
        x = input + self.drop_path(self.op(self.norm(input)))
        num_seg = self.config.SEG_QUERY_NUM
        query2 = x.reshape(1,180*180,-1)[:,top_index.reshape(-1),:].unsqueeze(1) # (b, 1, 230, d)
        if self.config.get('MAMBA_WITH_CA',False):
            w_index,h_index = top_index % 180, top_index//180

            if self.use_channel_gate:
                gate_det, gate_seg, gate_occ = self.gate(x)
                x_det,x_seg = x*gate_det,x*gate_seg

                h_feat_det = x_det[:,:,w_index[:200]].transpose(1,2) #(b,180,230,d) --> (b,230,180,d) 
                w_feat_det = x_det[:,h_index[:200]]  #(b,230,180, d)
                hw_feat_det = torch.cat([h_feat_det,w_feat_det],0)

                h_feat_seg = x_seg[:,:,w_index[200:200+num_seg]].transpose(1,2) #(b,180,230,d) --> (b,230,180,d) 
                w_feat_seg = x_seg[:,h_index[200:200+num_seg]]  #(b,230,180, d)
                hw_feat_seg = torch.cat([h_feat_seg,w_feat_seg],0)

                if self.config.TRAIN_OCC:
                    x_occ = x*gate_occ
                    h_feat_occ = x_occ[:,:,w_index[200+num_seg:]].transpose(1,2) #(b,180,230,d) --> (b,230,180,d) 
                    w_feat_occ = x_occ[:,h_index[200+num_seg:]]  #(b,230,180, d)
                    hw_feat_occ = torch.cat([h_feat_occ,w_feat_occ],0)
                    hw_feat = torch.cat([hw_feat_det,hw_feat_seg,hw_feat_occ],1)
                else:
                    hw_feat = torch.cat([hw_feat_det,hw_feat_seg],1)

            else:
                h_feat = x[:,:,w_index].transpose(1,2) #(b,180,230,d) --> (b,230,180,d) 
                w_feat = x[:,h_index]  #(b,230,180, d)
                hw_feat = torch.cat([h_feat,w_feat],0)
                # if self.config.TRAIN_OCC:
                #     query_occ = query2[:,:,200+num_seg:]*gate_occ[:,:,200+num_seg:]
                #     query2 = torch.cat([query_det,query_seg,query_occ],2)
                # else:
                #     query2 = torch.cat([query_det,query_seg],2)

            hw_feat = torch.cat([query2.transpose(1,2).repeat(2,1,1,1),hw_feat],2)

            query2 = self.op3(self.norm3(hw_feat.reshape(-1,1,181,query.shape[-1]))).reshape(2,query.shape[-2],181,query.shape[-1]).sum(0,True)
            query2 = query2[:,:,0:1].transpose(1,2)

        else:
            if self.use_channel_gate:
                if self.config.get('GATE_ONE_FFN_WITH_QUERY',False):
                    gate_det, gate_seg, gate_occ = self.gate(query)
                else:
                    gate_det, gate_seg, gate_occ = self.gate(query2)
                query_det = query2[:,:,:200]*gate_det[:,:,:200]
                query_seg = query2[:,:,200:200+num_seg]*gate_seg[:,:,200:200+num_seg]
                if self.config.TRAIN_OCC:
                    query_occ = query2[:,:,200+num_seg:]*gate_occ[:,:,200+num_seg:]
                    query2 = torch.cat([query_det,query_seg,query_occ],2)
                else:
        ...
        if self.config.get('QUERY_MAMBA2D',False):
            query= query + self.drop_path(self.op2(self.norm2(query2)))
        else:
            query= query + self.drop_path(self.norm2(query2))

        gate_seg_src = None
        if self.config.get('USE_TCS',False):
            if layer_index != self.config.SEM_SEG_HEAD.TRANSFORMER_ENC_LAYERS-1:
                gate_seg_src = None
            else:
                gate_seg_src = self.gate(x)[1]

        return x, query, gate_seg_src

    def forward(self, input, top_index, bg_index, query,layer_index):
        if self.use_checkpoint:
            return checkpoint.checkpoint(self._forward, input)
        else:
            return self._forward(input, top_index, bg_index, query, layer_index)

class VSSMENC(nn.Module):
    def __init__(
            self, 
            in_chans=256, 
            num_classes=1000, 
    ...
    def forward(self, x, top_index, bg_index, query):
        x_input = self.patch_embed(x)
        x = x_input
        for idx, layer in enumerate(self.layers):
            x, query, gate_seg = layer(x, top_index, bg_index, query, idx)
        out = x.permute(0,3,1,2).contiguous()
        if self.config.get('USE_MEMORY_AS_FEAT',False):
            map_bev = out
        else:
            if self.config.get('USE_TCS',False):
                map_bev = self.map_features(out+x_input.permute(0,3,1,2), gate_seg.permute(0,3,1,2))
            else:
                map_bev = self.map_features(out+x_input.permute(0,3,1,2), torch.ones_like(out))

        map_features = self.mapslicer(map_bev).transpose(2,3)
        # query_feat = out.flatten(2)[:,:,top_index.reshape(-1)].transpose(1,2)
        return query.squeeze(1), map_features
    ...
    def flops(self, shape=(512, 180, 180)):
        ...
        Gflops, unsupported = flop_count(model=model, 
                                         inputs=(input,torch.zeros(230).cuda().long(),torch.zeros(230).cuda().long(),torch.zeros([1,230,256]).cuda()), 
    ...
    if isinstance(results,list):
        ...
        for i in range(num):
            ...
            if len(results[i].shape)==2:
                plt.imshow(results[i].detach().cpu().numpy(),cmap='jet')
            else:
                plt.imshow(results[i].detach().sum(0).cpu().numpy(),cmap='jet')
        plt.show()
    else:
        if len(results.shape) ==2:
            plt.imshow(results.detach().cpu().numpy(),cmap='jet');plt.show()
        else:
            num=results.shape[0]
            # row = (num // 4) +1
            for i in range(num):
...
if __name__ == "__main__":
    import pdb; pdb.set_trace()
    a = VSSM().cuda()
    input_tensor = torch.zeros([1,256,180,180]).cuda().float()
    b = a(input_tensor)
    print(b.shape)
