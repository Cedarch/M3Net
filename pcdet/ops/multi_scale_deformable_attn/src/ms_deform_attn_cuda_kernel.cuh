...
__global__ void ms_deformable_col2im_gpu_kernel_shm_blocksize_aware_reduce_v1(
  ...
  CUDA_1D_KERNEL_LOOP(index, n) {
    ...
    scalar_t *grad_sampling_loc_cur = grad_sampling_loc + (grad_sampling_ptr << 1);
    scalar_t *grad_attn_weight_cur = grad_attn_weight + grad_sampling_ptr;
      ...
      for (int p_col = 0; p_col < num_point; ++p_col) {
        ...
        *(cache_grad_attn_weight + threadIdx.x) = 0;
        ...
        if (tid == 0) {
          ...
          *grad_sampling_loc_cur = _grad_w;
          *(grad_sampling_loc_cur + 1) = _grad_h;
          *grad_attn_weight_cur = _grad_a;
        }
        __syncthreads();

        data_weight_ptr += 1;
        data_loc_w_ptr += 2;
        grad_attn_weight_cur += grad_weight_stride;
        grad_sampling_loc_cur += grad_loc_stride;
      }
    }
  }
}
    ...
    for (int l_col = 0; l_col < num_levels; ++l_col) {
      ...
      for (int p_col = 0; p_col < num_point; ++p_col) {
        ...
        *(cache_grad_attn_weight + threadIdx.x) = 0;
        ...
        if (tid == 0) {
          *grad_sampling_loc_cur = cache_grad_sampling_loc[0];
          *(grad_sampling_loc_cur + 1) = cache_grad_sampling_loc[1];
          *grad_attn_weight_cur = cache_grad_attn_weight[0];
        }
        __syncthreads();
    ...
    for (int l_col = 0; l_col < num_levels; ++l_col) {
      ...
      for (int p_col = 0; p_col < num_point; ++p_col) {
        ...
        *(cache_grad_attn_weight + threadIdx.x) = 0;
        ...
        if (tid == 0) {
          atomicAdd(grad_sampling_loc_cur, cache_grad_sampling_loc[0]);
          atomicAdd(grad_sampling_loc_cur + 1, cache_grad_sampling_loc[1]);
          atomicAdd(grad_attn_weight_cur, cache_grad_attn_weight[0]);
        }
        __syncthreads();
      ...
      for (int p_col = 0; p_col < num_point; ++p_col) {
        ...
        if (h_im > -1 && w_im > -1 && h_im < spatial_h && w_im < spatial_w) {
          ms_deform_attn_col2im_bilinear_gm(
              data_value_ptr, spatial_h, spatial_w, num_heads, channels, h_im,
              w_im, m_col, c_col, top_grad, weight, grad_value_ptr,
              grad_sampling_loc_cur, grad_attn_weight_cur);
        }
        data_weight_ptr += 1;
        data_loc_w_ptr += 2;
        grad_attn_weight_cur += grad_weight_stride;
